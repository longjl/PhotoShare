 ...
 */
public class RecordsAsyncTask extends AsyncTask<Void, Void, List<Record>> {
    ...
    public static void execute(Context context, RecordResultListener listener, String acc_id) {
        new RecordsAsyncTask(context, listener, acc_id).execute();
    }

    private RecordsAsyncTask(Context context, RecordResultListener listener, String acc_id) {
        mContext = context;
        mListener = listener;
        mAccId = acc_id;
    }
